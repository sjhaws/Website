...
- **Enemies:** snakes and scorpions, plus guards on level 3, lions on levels 2, 4 and 5, monkeys throwing coconuts on level 5, and sharks and whales on level 6.
  ...
  - **Monkeys (level 5):** five palm trees stand along the ground (`PALM_SPOTS` in `game.js`), each nudged clear of the platforms so none hides its monkey, and a monkey sits in each crown. When Nephi is within 520 px across, the monkey turns to face him and throws a coconut, resting 0.9 to 2 seconds between throws. It throws one of two ways, pose by pose (`MONKEY_THROWS`): overhead (reaching for a coconut, holding it, raising it overhead, letting go) or a sidearm lob (reaching, holding, winding up, lunging forward, following through). Either way it recovers and then shades its eyes to watch where the coconut went. About a second passes between the reach and the throw, as warning. The coconut flies in an arc for 0.65 to 1.15 seconds, longer the farther it goes, aimed at where Nephi will be by then if he keeps going: stopping, turning or jumping dodges it, and a platform in the way stops it. A coconut that hits Nephi restarts the level; once it lands it's harmless, and it rolls to a stop and fades. A sword swing started up to about a fifth of a second before a coconut would hit knocks it away (`batCoconut`), as long as it's coming from in front of him or overhead. The monkeys can't be hurt. The palms (`Palm.webp`, shown at 3×) and the monkey's ten poses and the coconut (`Monkey.webp`, `Coconut.webp`, all facing right) come from pixel-art sheets supplied for the site, reduced to their true pixel grids and cleaned to one palette each; the loose coconuts drawn on the monkey sheet were taken out of its throwing poses.
...
- [ ] Level 5 has five palm trees, each with a monkey that faces Nephi and throws coconuts when he's near; a coconut restarts the level if it hits him, stopping or turning dodges it, and a well-timed sword swing knocks it away; its story card says so
